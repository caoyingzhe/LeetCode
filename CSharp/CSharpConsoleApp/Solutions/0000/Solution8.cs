...
namespace CSharpConsoleApp.Solutions
{
    ...
    class Solution8 : SolutionBase
    {
        ...
        public int MyAtoi(string s)
        {
            ...
            bool hasSign = IsSign(x[0]);
            if (x.Length > 1 && hasSign && IsSign(x[1])) //首字符是符号，第二个也是符号, 比如"=++23123", "-+123"
            {
                return 0;
            }
            else if(x.Length > 1 && hasSign && !IsNumber(x[1])) //首字符是符号,第二个不是数字, 比如"=ws23123", 
            {
                return 0;
            }
            else if(!hasSign && !IsNumber(x[0]))  //首字符不是符号，也不是数字  比如"=123123", "wer123"
            {
                return 0;
            }
            ...
            for (int i = startIndex; i < x.Length; i++)
            {
                ...
                {
                    int result;
                    if (int.TryParse(x.Substring(0, lastIndex + 1), out result))
                    {
                        return result;
                    }
                    else
                    {
                        return (x[0] == '-') ? int.MinValue : int.MaxValue;
                    }
                }
            }
            return 0;
